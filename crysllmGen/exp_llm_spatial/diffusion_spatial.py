"""SpatialCrossAttnCSPDiffusion: CSPDiffusion with spatial cross-attention decoder."""
import torch
...
from exp_llm_spatial.cspnet_spatial import SpatialCrossAttnCSPNet


class SpatialCrossAttnCSPDiffusion(CSPDiffusion):
    """Diffusion model using SpatialCrossAttnCSPNet decoder."""

    def __init__(self, timesteps, run_type, use_llm_cond=True):
        super(CSPDiffusion, self).__init__()

        self.decoder = SpatialCrossAttnCSPNet(
            ...
            llm_feat_dim=4096, num_tokens=8, spatial_num_freqs=4,
        )
    ...
    def sample(self, batch, step_lr=1e-5, diff_steps=1000, llm_feat=None):
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            traj[t - 1] = {
                'num_atoms': batch.num_atoms, 'atom_types': batch.atom_types,
                'frac_coords': x_t_minus_1 % 1., 'lattices': l_t_minus_1}

        traj_stack = {
            'num_atoms': traj[0]['num_atoms'], 'atom_types': traj[0]['atom_types'],
            'frac_coords': traj[0]['frac_coords'], 'lattices': traj[0]['lattices']}
