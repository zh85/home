"""Two-phase sampling for Spatial Cross-Attention conditioned generation.

Phase 1: LLM generation + feature extraction → save to disk
Phase 2: Load saved data → diffusion sampling → save results

Usage:
    # Phase 1 only
    python exp_llm_spatial/crysllmgen_sample_spatial.py --phase 1 --num_samples 10000

    # Phase 2 only (after Phase 1 completes)
    python exp_llm_spatial/crysllmgen_sample_spatial.py --phase 2 --chkpt_name out/mp_20/.../model_XXX.pt
"""
...
def parse_generated_text(gen_text):
    ...
    structure = Structure(lattice=Lattice.from_parameters(*(lengths_val + angles_val)),
                          species=species, coords=coords, coords_are_cartesian=False)
    return structure.to(fmt="cif")


def resize_tokenizer_and_embeddings(tokenizer, model, special_tokens):
...
class SampleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return Data(num_atoms=torch.LongTensor([s["n_atom"]]), num_nodes=s["n_atom"],
...
def lattices_to_params_shape(lattices):
    ...
    for i in range(3):
        j, k = (i + 1) % 3, (i + 2) % 3
        cos_angle = torch.sum(lattices[..., j, :] * lattices[..., k, :], dim=-1) / (lengths[..., j] * lengths[..., k])
        angles[..., i] = torch.clamp(cos_angle, -1.0, 1.0)
    angles = torch.arccos(angles) * 180.0 / np.pi
    return lengths, angles


def phase1_llm_generation(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(args.llama_model_path, torch_dtype=torch.bfloat16, device_map="auto")
    special_tokens = {}
    ...
    collected_data, collected_features = [], []
        ...
        for gen_text, prompt in zip(decoded, batch_prompts):
            ...
            try:
                ...
                (frac_coords, atom_types, lengths, angles,
                 num_atoms, _, _, data_dict) = process_one(cif_str, True, False, "crystalnn", False, 0.01)
            except Exception:
                continue
    ...
    print(f"Saved to {out_prefix}_data.pt and {out_prefix}_features.pt")
    del model, tokenizer
    torch.cuda.empty_cache()


def phase2_diffusion(args):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=== Phase 2: Diffusion Sampling ===")
    ...
    diffusion_model = SpatialCrossAttnCSPDiffusion(args.timesteps, args.run_type, use_llm_cond=True).to(device)
    ...
    dataloader = DataLoader(dataset, batch_size=args.diff_batch_size, shuffle=False)
    ...
    torch.save({'frac_coords': frac_coords_all, 'num_atoms': num_atoms_all,
                'atom_types': atom_types_all, 'lengths': lengths, 'angles': angles,
                'time': time.time() - start_time}, output_file)
...
def main():
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="LLM generation batch size")
    parser.add_argument("--diff_batch_size", type=int, default=32, help="Diffusion sampling batch size")
    parser.add_argument("--temperature", type=float, default=0.9)
    parser.add_argument("--top_p", type=float, default=0.9)
    parser.add_argument("--chkpt_name", type=str, default="out/mp_20/model_final.pt")
    ...
    parser.add_argument("--out-prefix", type=str, default="results/llama3_sample_spatial")
    parser.add_argument("--use_llm_cond", action="store_true", default=True)
    args = parser.parse_args()

    if args.phase == 1:
        phase1_llm_generation(args)
    else:
        phase2_diffusion(args)
