"""Training script for Spatial Cross-Attention diffusion model.

Usage:
    python exp_llm_spatial/diff_train_spatial.py --dataset mp_20 --epochs 500 --use_llm_cond
"""
import sys, os
...
from exp_llm_spatial.diffusion_spatial import SpatialCrossAttnCSPDiffusion
...
def train(train_dataloader, val_dataloader, test_dataloader,
    ...
    for epoch in tqdm(range(epochs)):
        t0 = time()
        loss, coord_loss, lattice_loss = train_epoch(train_dataloader, model, use_llm_cond)
        val_loss, val_coord_loss, val_lattice_loss = validate(val_dataloader, model, use_llm_cond)
        ...
        if epoch % 10 == 0:
            print(f"Epoch {epoch}/{epochs} Loss: {loss:.4f}, Coord Loss: {coord_loss:.4f}, Lattice Loss: {lattice_loss:.4f} [{time()-t0:.2f}s]", best_str)
            print(f"Validation Coord Loss: {val_coord_loss:.4f}, Lattice Loss: {val_lattice_loss:.4f}, [{time()-t0:.2f}s]\n")
            results = ('Epoch :' + str(epoch) + ' Train Diff Loss : ' + str(round(loss,4))
    ...
    test_lattice_loss, test_coord_loss, test_type_loss = validate(test_dataloader, model, use_llm_cond)
    print(f"Test Coord Loss: {test_coord_loss:.4f}, Lattice Loss: {test_lattice_loss:.4f}, [{time()-t0:.2f}s]")
    test_results = ('Test Coord Loss : ' + str(round(test_coord_loss,4)) + ' Test Lattice Loss : ' + str(round(test_lattice_loss,4)))
...
def main(args):
    ...
    scaler_path = f"{llm_feat_dir}/llm_feat_scaler.pt" if args.use_llm_cond else None
    dataset_kwargs = {}
    if args.use_llm_cond:
        dataset_kwargs = {
            'train': {'llm_feat_path': f"{llm_feat_dir}/llm_feat_train_norm.pt", 'llm_scaler_path': scaler_path},
            'val': {'llm_feat_path': f"{llm_feat_dir}/llm_feat_val_norm.pt", 'llm_scaler_path': scaler_path},
            'test': {'llm_feat_path': f"{llm_feat_dir}/llm_feat_test_norm.pt", 'llm_scaler_path': scaler_path},
        }
    train_dataset = CondMaterialDataset(os.path.join(data_base, f"{config.train_data}.csv"), **dataset_kwargs.get('train', {}))
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, pin_memory=True)
    val_dataset = CondMaterialDataset(os.path.join(data_base, f"{config.eval_data}.csv"), **dataset_kwargs.get('val', {}))
    val_dataloader = DataLoader(val_dataset, batch_size=32, shuffle=True, pin_memory=True)
    test_dataset = CondMaterialDataset(os.path.join(data_base, f"{config.test_data}.csv"), **dataset_kwargs.get('test', {}))
    test_dataloader = DataLoader(test_dataset, batch_size=32, shuffle=True, pin_memory=True)

    model = SpatialCrossAttnCSPDiffusion(args.timesteps, args.run_type, use_llm_cond=args.use_llm_cond).to(device)
    train(train_dataloader, val_dataloader, test_dataloader, model, args.epochs, args.dataset, use_llm_cond=args.use_llm_cond)
